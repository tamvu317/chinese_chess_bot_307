...
def merge_with_reference(current_points, reference_points, pieces_list, yolo_pieces):
    ...
    for ref_pt in sorted(reference_points, key=lambda p: p["label"]):
        ...
        for px, py, _ in pieces_list:
            dist = math.hypot(px - ref_pt["px"], py - ref_pt["py"])
            if dist < best_dist:
                best_dist = dist
                nearest_piece = (px, py)
...
while True:
    ...
    if key == ord('q'): break
        ...
        if best_move:
            ...
            label_to_xy = {pt['label']: (pt.get('rob_x'), pt.get('rob_y')) for pt in mapped_points}
            start_xy = label_to_xy.get(start_label)
            end_xy = label_to_xy.get(end_label)
